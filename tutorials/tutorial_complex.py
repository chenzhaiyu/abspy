...
def example_cell_complex_from_planes():
    ...
    colours = np.zeros([num_representatives * cell_complex.num_cells, 4], dtype=np.uint8)
    ...
    representatives_vis.export(dir_tests / 'output' / 'test_vis.ply', file_type='ply')
    try:
    ...
    cell_complex.save(dir_tests / 'output' / 'complex.cc')

    # save cells to OBJ and PLM files
    cell_complex.save_obj(dir_tests / 'output' / 'cells.obj', use_mtl=True)
    cell_complex.save_plm(dir_tests / 'output' / 'cells.plm')
...
def example_cell_complex_from_mesh():
    ...
    vertex_group_reference = VertexGroupReference(filepath=dir_tests / 'data' / 'test_church.obj', num_samples=10000)
    ...
    cells_in_mesh = cell_complex.cells_in_mesh(dir_tests / 'data' / 'test_church.obj', engine='distance')

    # save cell complex CC file
    cell_complex.save(dir_tests / 'output' / 'test_complex.cc')

    #  Save cells to OBJ and PLM files
    cell_complex.save_obj(dir_tests / 'output' / 'test_cells.obj', use_mtl=True, indices_cells=cells_in_mesh)
    cell_complex.save_plm(dir_tests / 'output' / 'test_cells.plm')

    # visualise the inside cells (only if pyglet installation is found and valid indices are provided)
    if len(cells_in_mesh):
        try:
            cell_complex.visualise(indices_cells=cells_in_mesh, temp_dir='./output/')
        except (Exception, ImportError) as e:
            print(f'visualization skipped: {e}')
