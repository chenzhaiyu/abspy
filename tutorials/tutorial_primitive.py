...
def example_extract_primitives_vg():
    ...
    vertex_group.save_planes_npy(dir_tests / 'output' / 'primitives_planes_vg.npy')
    vertex_group.save_aabbs_npy(dir_tests / 'output' / 'primitives_bounds_vg.npy')

    # save extracted primitives to a Vertex group (.vg) file
    vertex_group.save_vg(dir_tests / 'output' / 'primitives.vg')
...
def example_extract_primitives_bvg():
    ...
    Extract primitives from VertexGroup (.vg) file.
    """
    vertex_group = VertexGroup(filepath=dir_tests / 'data' / 'test_points.bvg')
    ...
    # save planes and bounds as numpy (.npy) files
    vertex_group.save_planes_npy(dir_tests / 'output' / 'primitives_planes_bvg.npy')
    vertex_group.save_aabbs_npy(dir_tests / 'output' / 'primitives_bounds_bvg.npy')

    # save extracted primitives to a binary Vertex group (.bvg) file
    vertex_group.save_bvg(dir_tests / 'output' / 'primitives.bvg')
...
def example_extract_reference_primitives():
    ...
    """
    vertex_group_reference = VertexGroupReference(filepath=dir_tests / 'data' / 'test_mesh.ply', num_samples=10000)
    ...
    # save point cloud
    vertex_group_reference.save_cloud(dir_tests / 'output' / 'reference.ply')

    # save extracted primitives to both a Vertex Group (.vg) file and a binary Vertex group (.bvg) file
    vertex_group_reference.save_vg(dir_tests / 'output' / 'reference.vg')
    vertex_group_reference.save_bvg(dir_tests / 'output' / 'reference.bvg')
